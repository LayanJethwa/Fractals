...
if __name__ == "__main__":
    ...
    while running:
        ...
        if not flat:
            ...
            proj_x = np.zeros_like(height_map, dtype=np.int32)
            proj_y = np.zeros_like(height_map, dtype=np.int32)
            for y in range(0, res - step, step):
                for x in range(0, res - step, step):
                    sx, sy = project_iso(x, y, height_map[y,x], theta)
                    sx = np.clip(sx, -1000, 1000)
                    sy = np.clip(sy, -1000, 1000)
                    proj_x[y,x] = sx
                    proj_y[y,x] = sy

            for y in range(0, res - step, step):
                for x in range(0, res - step, step):
                    z1, z2 = height_map[y, x], height_map[y, x+step]
                    z3, z4 = height_map[y+step, x+step], height_map[y+step, x]
                    avg_z = (z1 + z2 + z3 + z4) / 4

                    if avg_z != 0:
                        p1 = (proj_x[y,x], proj_y[y,x])
                        p2 = (proj_x[y,x+step], proj_y[y,x+step])
                        p3 = (proj_x[y+step,x+step], proj_y[y+step,x+step])
                        p4 = (proj_x[y+step,x], proj_y[y+step,x])

                        avg_h = int((hm_int[y, x] + hm_int[y, x+step] + hm_int[y+step, x+step] + hm_int[y+step, x]) / 4)
                        colour = colours[avg_h]

                        triangles.append((p1, p2, p3, colour, avg_z))
                        triangles.append((p1, p3, p4, colour, avg_z))
            triangles.sort(key=lambda t: t[4])

            screen.fill((255,0,0))
            for tri in triangles:
                p1, p2, p3, colour, avg_z = tri
